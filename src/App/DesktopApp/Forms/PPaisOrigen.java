...
public class PPaisOrigen extends JPanel implements ActionListener {
    private transient FactoryBL<PaisOrigenDTO> blFactory = new FactoryBL<>(PaisOrigenDAO.class);
    private transient PaisOrigenDTO dtoPais = new PaisOrigenDTO();
    ...
    private void loadRowData() throws Exception {
        regAct = blFactory.getMinReg("idPaisOrigen");
        regMax = blFactory.getMaxReg("idPaisOrigen");
        dtoPais = blFactory.getBy(regAct);
    }

    private void showRowData() {
        boolean isDTONull = (dtoPais == null || dtoPais.getIdPaisOrigen() == null);
        txtIdPais.setText((isDTONull) ? " " : dtoPais.getIdPaisOrigen().toString());
    ...
    private void showDataTable() throws Exception {
        ...
        int index = 0;
        for (PaisOrigenDTO d : blFactory.getAll()) {
            data[index][0] = d.getIdPaisOrigen();
    ...
    private void btnGuardarClick() {
        ...
        try {
            if (AppMSG.showConfirmYesNo("¿Seguro que desea " + ((isDTONull) ? "AGREGAR ?" : "ACTUALIZAR ?"))) {
                if (isDTONull)
                    dtoPais = new PaisOrigenDTO(txtNombre.getText().trim());
                else
    ...
    private void btnEliminarClick() {
        try {
            if (AppMSG.showConfirmYesNo("Seguro que desea Eliminar?")) {
                ...
                if (!blFactory.del(dtoPais.getIdPaisOrigen()))
